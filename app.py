...
html, body, .stApp { background: #080c10 !important; color: #e2e8f0 !important; font-family: 'DM Mono', monospace !important; font-size: 16px !important; }
.stApp::before { content: ''; position: fixed; inset: 0; background-image: linear-gradient(rgba(0,212,170,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,212,170,0.03) 1px, transparent 1px); background-size: 40px 40px; pointer-events: none; z-index: 0; }
section[data-testid="stSidebar"] { background: #0d1117 !important; border-right: 1px solid #1a2332 !important; }
#MainMenu, footer, header { visibility: hidden !important; }
.stDeployButton { display: none !important; }
.stTextInput label, .stNumberInput label, .stSelectbox label, .stSlider label { font-size: 16px !important; color: #94a3b8 !important; font-weight: 500 !important; }
.stTextInput input, .stNumberInput input { background: #0d1117 !important; border: 1px solid #1e2d3d !important; border-radius: 6px !important; color: #e2e8f0 !important; font-family: 'DM Mono', monospace !important; font-size: 16px !important; }
.stButton > button { background: linear-gradient(135deg, #00d4aa, #00a88a) !important; color: #080c10 !important; border: none !important; border-radius: 6px !important; font-family: 'DM Mono', monospace !important; font-weight: 600 !important; font-size: 15px !important; padding: 0.55rem 1.2rem !important; transition: all 0.2s !important; }
.stButton > button:hover { background: linear-gradient(135deg, #00ffcc, #00d4aa) !important; transform: translateY(-1px) !important; }
[data-testid="metric-container"] { background: #0d1117 !important; border: 1px solid #1e2d3d !important; border-radius: 10px !important; padding: 1rem !important; }
.chat-user { background: linear-gradient(135deg, rgba(0,212,170,0.12), rgba(0,212,170,0.06)); border: 1px solid rgba(0,212,170,0.25); border-radius: 14px 14px 2px 14px; padding: 0.9rem 1.1rem; margin: 0.5rem 0 0.5rem 4rem; font-size: 16px; line-height: 1.7; color: #e2e8f0; }
.chat-ai { background: #0d1117; border: 1px solid #1e2d3d; border-radius: 14px 14px 14px 2px; padding: 0.9rem 1.1rem; margin: 0.5rem 4rem 0.5rem 0; font-size: 16px; line-height: 1.7; color: #cbd5e1; }
.chat-label-user { text-align: right; font-size: 12px; color: #00d4aa; margin-bottom: 0.2rem; letter-spacing: 0.1em; }
.chat-label-ai { font-size: 12px; color: #4a6741; margin-bottom: 0.2rem; letter-spacing: 0.1em; }
.page-title { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 2rem; color: #f1f5f9; letter-spacing: -0.03em; margin-bottom: 0.2rem; }
.page-sub { font-size: 13px; color: #4a6741; letter-spacing: 0.12em; margin-bottom: 1rem; }
.disclaimer { background: #0d1a0d; border: 1px solid #1a3a1a; border-left: 3px solid #22c55e; border-radius: 6px; padding: 0.75rem 1rem; font-size: 14px; color: #4ade80; margin-bottom: 1.2rem; line-height: 1.5; }
.card { background: #0d1117; border: 1px solid #1e2d3d; border-radius: 10px; padding: 1.3rem; margin-bottom: 1rem; }
.card-title { font-family: 'Syne', sans-serif; font-size: 13px; color: #4a6741; letter-spacing: 0.14em; text-transform: uppercase; margin-bottom: 0.9rem; }
.quick-q { display: block; width: 100%; text-align: left; background: #111820; border: 1px solid #1e2d3d; border-radius: 6px; color: #64748b; font-family: 'DM Mono', monospace; font-size: 14px; padding: 0.55rem 0.8rem; cursor: pointer; margin-bottom: 0.4rem; transition: all 0.15s; }
table { width: 100%; border-collapse: collapse; font-size: 15px; }
th { background: #111820; color: #4a6741; font-weight: 500; padding: 0.6rem 0.8rem; text-align: left; font-size: 13px; text-transform: uppercase; border-bottom: 1px solid #1e2d3d; }
td { padding: 0.6rem 0.8rem; border-bottom: 1px solid #111820; color: #cbd5e1; }
tr:hover td { background: #0a1628; }
.sip-result { background: linear-gradient(135deg, #0d2018, #0d1a2a); border: 1px solid #1a3a2a; border-radius: 14px; padding: 2rem; text-align: center; }
.sip-amount { font-family: 'Syne', sans-serif; font-size: 2.6rem; font-weight: 800; color: #00d4aa; letter-spacing: -0.04em; }
...
.logo-text { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 1.2rem; color: #00d4aa; }
...
# ─── SEBI PROMPT ──────────────────────────────────────────────────────────────
SEBI_PROMPT = """You are a SEBI registered Investment Adviser AI named InvestAI. Help Indian investors with investment planning, mutual funds, stocks, SIP, tax-saving investments, and financial goals.

RULES:
- Always add disclaimer about market risks
- Never give direct buy/sell recommendations
- Never promise guaranteed returns
- Be helpful and use simple Hindi/English mix
...
def render_sidebar():
    with st.sidebar:
        st.markdown('<div style="padding:1.5rem;border-bottom:1px solid #1a2332;"><div class="logo-text">InvestAI</div><div class="logo-sub">SEBI REGISTERED ADVISER</div></div>', unsafe_allow_html=True)
        ...
        st.markdown('<div style="position:fixed;bottom:1rem;left:0;width:220px;padding:0 1.5rem;"><div style="font-size:12px;color:#1e2d3d;line-height:1.6;">SEBI Reg. No: INA000000000<br>2026 InvestAI Advisory</div></div>', unsafe_allow_html=True)

# ─── LOGIN ────────────────────────────────────────────────────────────────────
def page_login():
    c1, c2, c3 = st.columns([1, 1.2, 1])
    with c2:
        st.markdown('<div style="background:#0d1117;border:1px solid #1e2d3d;border-radius:16px;padding:2.5rem;margin-top:3rem;"><div style="font-family:Syne,sans-serif;font-weight:800;font-size:2rem;color:#00d4aa;text-align:center;">InvestAI</div><div style="text-align:center;font-size:12px;color:#4a6741;letter-spacing:0.18em;margin-bottom:2rem;">SEBI REGISTERED INVESTMENT ADVISER</div>', unsafe_allow_html=True)
        ...
        with qc:
            if st.button("Viewer", use_container_width=True, key="qf_viewer"):
        ...
        password = st.text_input("Password", value=st.session_state.get('_qf_pass',''), type="password", placeholder="password")
...
def page_chat():
    ...
    st.markdown('<div class="disclaimer">Warning: Investments are subject to market risks. Please read all scheme related documents carefully before investing.</div>', unsafe_allow_html=True)

    col1, col2 = st.columns([2.2, 1])

    with col2:
        st.markdown('<div class="card"><div class="card-title">Quick Questions</div>', unsafe_allow_html=True)
        for q in ["SIP ke baare mein batao", "Tax saving investment kya hain?", "ELSS vs PPF comparison", "Market crash mein kya karein?", "Mutual fund kaise choose karein?"]:
            if st.button(f"-> {q}", key=f"qq_{q}", use_container_width=True):
                st.session_state.messages.append({"role": "user", "content": q})
                with st.spinner("AI soch raha hai..."):
                    reply = ask_groq(q, st.session_state.history)
                st.session_state.messages.append({"role": "ai", "content": reply})
                st.session_state.history.extend([{"role": "user", "content": q}, {"role": "assistant", "content": reply}])
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        with st.expander(f"Holdings ({len(st.session_state.holdings)})", expanded=False):
            ...
            with c1h:
                ...
                h_buy = st.number_input("Buy Rs.", min_value=0.0, key="ch_buy")
            with c2h:
                h_curr = st.number_input("Current Rs.", min_value=0.0, key="ch_curr")
...
def page_portfolio():
    ...
    st.markdown('<div class="page-sub">AI-POWERED PORTFOLIO ANALYSIS</div>', unsafe_allow_html=True)
    st.markdown('<div class="disclaimer">Warning: Investments are subject to market risks.</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        with c1p:
            ...
            p_buy = st.number_input("Buy Price Rs.", min_value=0.0, key="p_buy")
        with c2p:
            p_curr = st.number_input("Current Rs.", min_value=0.0, key="p_curr")
    ...
    with col1:
        if not st.session_state.holdings:
            st.markdown('<div style="text-align:center;padding:3rem;background:#0d1117;border:1px dashed #1e2d3d;border-radius:10px;"><div style="font-size:3rem;margin-bottom:1rem;">Portfolio</div><div style="color:#4a6741;font-size:15px;">Right side se holdings add karo</div></div>', unsafe_allow_html=True)
        else:
            ...
            for h in st.session_state.holdings:
                ...
                tbl += f"<tr><td style='color:#e2e8f0;font-weight:500;'>{h['name']}</td><td style='color:#64748b;'>{h['asset_type']}</td><td>{h['qty']}</td><td>{fmt(h['buy_price'])}</td><td>{fmt(h['current_price'])}</td><td style='color:{c};'>{a} {fmt(abs(pl))} ({pp:.1f}%)</td></tr>"
            tbl += "</table>"
            ...
            with b1:
                if st.button("AI Analysis Karo", use_container_width=True):
                    portfolio_text = "\n".join([f"{h['name']}: {h['qty']} shares, buy Rs.{h['buy_price']}, current Rs.{h['current_price']}, type: {h['asset_type']}" for h in st.session_state.holdings])
                    prompt = f"Analyze this Indian investor portfolio:\n{portfolio_text}\n\nTotal invested: {fmt(total_inv)}, Current: {fmt(total_curr)}, P&L: {fmt(pnl)} ({pnl_pct:.1f}%)\n\nGive recommendations."
...
def page_sip():
    ...
    with col1:
        st.markdown('<div class="card"><div class="card-title">SIP Parameters</div>', unsafe_allow_html=True)
        monthly = st.number_input("Monthly Investment (Rs.)", min_value=500, max_value=1000000, value=10000, step=500)
        ...
        if st.button("Calculate Returns", use_container_width=True):
            ...
            st.markdown(f'<div class="sip-result"><div style="font-size:13px;color:#4a6741;letter-spacing:0.15em;margin-bottom:0.5rem;">MATURITY VALUE</div><div class="sip-amount">{fmt(maturity)}</div><div style="margin-top:1.5rem;display:flex;gap:2rem;justify-content:center;"><div><div style="font-size:12px;color:#4a6741;">INVESTED</div><div style="font-size:1.2rem;color:#e2e8f0;font-weight:600;">{fmt(invested)}</div></div><div><div style="font-size:12px;color:#4a6741;">GAINS</div><div style="font-size:1.2rem;color:#22c55e;font-weight:600;">{fmt(gains)}</div></div><div><div style="font-size:12px;color:#4a6741;">RETURNS</div><div style="font-size:1.2rem;color:#00d4aa;font-weight:600;">{((gains/invested)*100):.0f}%</div></div></div></div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        for rv in [6,8,10,12,15,18]:
            ...
            tbl+=f"<tr><td style='color:{c};'>{rv}%</td><td style='color:{c};'>{fmt(mat)}</td><td style='color:#22c55e;'>{fmt(g)}</td></tr>"
        tbl+="</table>"
        st.markdown(tbl, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="card"><div class="card-title">SIP Tips</div>', unsafe_allow_html=True)
        for t in ["Monthly SIP - discipline zaroori hai","Long term mein compounding ka jaadu","Har saal SIP 10-15% step-up karo","Alag goals ke liye alag SIP","Market crash mein SIP band mat karo"]:
